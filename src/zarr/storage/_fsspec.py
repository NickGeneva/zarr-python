    ...
    async def get(
        ...
        try:
            if byte_range is None:
                value = prototype.buffer.from_bytes(await self.fs._cat(path))
                ...
                value = prototype.buffer.from_bytes(
                    await self.fs._cat(path, start=byte_range.offset, end=None)
                )
            elif isinstance(byte_range, SuffixByteRequest):
                value = prototype.buffer.from_bytes(
                    await self.fs._cat(path, start=-byte_range.suffix, end=None)
                )
            else:
